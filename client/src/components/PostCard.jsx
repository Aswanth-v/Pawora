...
return(
    ...
    <div>
      <Link to={'/profile/'+reply?.userId?._id}>
      <p className='font-medium text-base text-ascent-1'>{reply?.userId?.firstName}{reply?.userId?.lastName}</p></Link>
      ...
      <div className='flex items-end justify-end pb-2'>
        {loading ? (
          ...
          <CustomButton
            title='Submit'
            type='submit'
            containerStyle='bg-[#8B4FB3] text-white py-1 px-3 rounded-full font-semibold text-sm'
          />
        )}
      </div>
    </form>
      ...
      />
      {
        loading ? (<Loading/>) : (
          comments?.length > 0 ? (
                ...
                <div className='ml-12'>
                  ...
                  <div className='mt-2 flex gap-6'>
                    <p className='flex gap-2 items-center text-base text-ascent-2 cursor-pointer'> 
                      ...
                      onClick={()=>SetReplyCommants(comment?._id)}
                      >
                        Reply
                      </span>
                  </div>
                      {replyCommants === comment?._id && (
                        <CommentForm
                        user={user}
                        id={comment?._id}
                ...
                <div className='py-2 px-8 mt-6'>
                  {comment?.replies?.length > 0 && (
                    <p
                      ...
                      onClick={() =>
                        SetShowReplay(
                          showReplay === comment?.replies?._id
                            ? 0
                            : comment?.replies?._id
                        )
                      }
            ...
            <span className='flex text-sm py-4 text-ascent-2 text-center'>No comments yet</span>
          )
        )
      }
      </div>
